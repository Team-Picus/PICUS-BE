...
import com.picus.core.expert.adapter.in.web.mapper.RequestApprovalResponseWebMapper;
...
import com.picus.core.expert.adapter.in.web.mapper.RequestApprovalRequestWebMapper;
...
public class RequestApprovalController {
    ...
    public BaseResponse<RequestApprovalWebResponse> requestApproval(@RequestBody RequestApprovalWebRequest webRequest) {
        ...
        Expert request = requestWebMapper.toDomain(webRequest); // 웹 요청 -> 어플리케이션 요청
        Expert response = requestApprovalUseCase.requestApproval(request); // 유스케이스 호출
        RequestApprovalWebResponse webResponse = responseWebMapper.toWebResponse(response); // 어플리케이션 응답 -> 웹 응답
